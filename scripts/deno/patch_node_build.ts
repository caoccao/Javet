 ...
 * 5. Run `make -j4` under `root_path_to_node_js` with the following exported.
 *
 *    export CFLAGS="-fPIC -ftls-model=global-dynamic -Wno-return-type"
 *    export CXXFLAGS="${CFLAGS}"
 *    export LDFLAGS="${CFLAGS}"
 *
 *    GYP appends them to every target of the target toolset via `CFLAGS.target`,
 *    `CXXFLAGS.target` and `LDFLAGS.target`, so the generated make files no
 *    longer need to be patched for position independent code.
 */
